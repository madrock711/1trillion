...
SUMMARY='미국 반도체 ETF가 5% 안팎 급락한 뒤 국내 메모리주는 장전부터 등락을 거듭하고 있습니다. KOSPI 6,650선 방어와 외국인 매도 완화가 반등의 지속성을 가릅니다.'
...
def main(seal):
    assert not (E/'seal-revision.json').exists(),'봉인 뒤에는 새 판본을 만든다.'
    ...
    report_path='reports/2026-09-15-'+stamp.strftime('%H%M')+'.md'
    ...
    d['scenarios']=[{'id':'base','label':'기본 · 45%','range':'KOSPI 6,650~6,800','summary':'전일 저점 부근에서 매도를 소화하며 반등을 시도합니다.','conditions':['6,650 방어','6,800 이하','반도체 낙폭 축소'],'invalidation':'6,650 이탈·6,800 돌파 또는 외국인 순매도 1조원 초과'}, {'id':'bull','label':'강세 · 20%','range':'KOSPI 6,800 초과~6,950','summary':'메모리주 반등과 외국인 매수로 6,800선을 회복합니다.','conditions':['6,800 초과','반도체 동반 회복','외국인·프로그램 순매수'],'invalidation':'6,800 아래 재진입'}, {'id':'bear','label':'약세 · 35%','range':'KOSPI 6,400~6,650 미만','summary':'반도체 매도가 이어지며 전일 저점 아래로 밀립니다.','conditions':['6,650 미만','반도체 약세 지속','환율 상승'],'invalidation':'6,650 회복'}]
    ...
    for path,cls in [('index.html','article-card home-article-card'),('articles/index.html','article-card'),('articles/market.html','market-article-item')]:
        ...
        card=m.group();t=t[:m.start()]+update(card).replace('미국 반도체 ETF가 5% 안팎 하락했습니다. 이미 급락한 한국장에서는 같은 뉴스를 두 번 세기보다 메모리주 장전 가격과 외국인 매도 완화를 살펴야 합니다.',SUMMARY)+t[m.end():]
        t=re.sub(r'2026-09-15T08:[0-9:]+\+09:00',issued,t)
        t=re.sub(r'2026.09.15 · 08:\d+ KST','2026.09.15 · '+stamp.strftime('%H:%M')+' KST',t)
        def schema(m):
            o=json.loads(m[1])
            def walk(v):
                if isinstance(v,dict):
                    if v.get('@type')=='ItemList' and v.get('itemListElement'):
                        v['itemListElement'][0]=json.loads(update(json.dumps(v['itemListElement'][0],ensure_ascii=False)))
    ...
    if seal:
        ...
        f['supersedes']='2026-09-15-0813-same-close'
        ...
        dump('research/evidence/2026-09-15/seal-revision.json',{'forecastId':fid,'issuedAt':issued,'dataCutoffAt':cutoff,'snapshotId':sid,'files':files})
