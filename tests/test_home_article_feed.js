...
assert.match(home, /<meta property="og:image" content="https:\/\/www\.hpmplab\.com\/assets\/images\/articles\/market-2026-08-27-nvidia-hbm-demand-1200x630\.webp">/);
...
assert.equal(cardTags.length, 39);
assert.equal((home.match(/<h3 class="home-article-title">/g) || []).length, 39);
assert.deepEqual(
    cardTags.reduce((counts, match) => ({...counts, [match[1]]: (counts[match[1]] || 0) + 1}), {}),
    {market: 24, health: 9, essay: 6}
);
...
assert.equal((home.match(/loading="lazy"/g) || []).length, 38);
...
assert.deepEqual(collection.mainEntity.itemListElement.map((item) => item.position), Array.from({length: 39}, (_, index) => index + 1));
assert.deepEqual(
...
assert.match(marketHero, /aria-label="시황 글"[\s\S]*>일일시황 <span class="article-category-count">24<\/span>/);
assert.doesNotMatch(marketHero, />전체 |\?category=essay|\?category=health/);
assert.equal((market.match(/<article class="market-article-item">/g) || []).length, 24);

const detailFiles = fs.readdirSync(path.join(root, 'articles'))
    .filter((name) => name.endsWith('.html') && name !== 'index.html' && name !== 'market.html');
assert.equal(detailFiles.length, 39);
...
for (const pathname of ['', 'articles/', 'articles/market.html']) {
    ...
    assert.match(sitemap, new RegExp(`<loc>https:\\/\\/www\\.hpmplab\\.com\\/${escaped}<\\/loc><lastmod>2026-08-27<\\/lastmod>`));
}
