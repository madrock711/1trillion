...
assert.match(home, /<meta property="og:image" content="https:\/\/www\.hpmplab\.com\/assets\/images\/articles\/market-2026-08-26-chip-rebound-event-gate-1200x630\.webp">/);
...
assert.equal(cardTags.length, 38);
assert.equal((home.match(/<h3 class="home-article-title">/g) || []).length, 38);
assert.deepEqual(
    cardTags.reduce((counts, match) => ({...counts, [match[1]]: (counts[match[1]] || 0) + 1}), {}),
    {market: 23, health: 9, essay: 6}
);
...
assert.equal((home.match(/loading="lazy"/g) || []).length, 37);
...
assert.deepEqual(collection.mainEntity.itemListElement.map((item) => item.position), Array.from({length: 38}, (_, index) => index + 1));
assert.deepEqual(
...
assert.match(marketHero, /aria-label="시황 글"[\s\S]*>일일시황 <span class="article-category-count">23<\/span>/);
assert.doesNotMatch(marketHero, />전체 |\?category=essay|\?category=health/);
assert.equal((market.match(/<article class="market-article-item">/g) || []).length, 23);

const detailFiles = fs.readdirSync(path.join(root, 'articles'))
    .filter((name) => name.endsWith('.html') && name !== 'index.html' && name !== 'market.html');
assert.equal(detailFiles.length, 38);
...
for (const pathname of ['', 'articles/', 'articles/market.html']) {
    ...
    assert.match(sitemap, new RegExp(`<loc>https:\\/\\/www\\.hpmplab\\.com\\/${escaped}<\\/loc><lastmod>2026-08-26<\\/lastmod>`));
}
