...
assert(dashboard.includes('function rememberTechnicalRangePreview(svg, rows, viewBox, series)'));
assert(dashboard.includes('function renderTechnicalLinePreview(svgId)'));
...
assert(dashboard.includes('var entryMargin = 6;'));
assert(dashboard.includes('var exitMargin = 12;'));
...
assert(dashboard.includes("slot.style.height = Math.ceil(navigatorHeight) + 'px'"), '플로팅 중 원래 슬롯 높이를 보존해 경계에서 레이아웃이 움직이지 않아야 한다.');
...
assert(previewBody.includes("['kospi-flow', 'kospi-flow-chart']"));
assert(previewBody.includes("['kodex-history', 'kodex-history-chart']"));
assert(previewBody.includes("['composite-momentum', 'composite-momentum-chart']"));
assert(previewBody.includes("['tqqq-history', 'tqqq-history-chart']"));
assert(previewBody.includes('renderTechnicalLinePreview(target[1])'), '드래그 중에는 경량 선 미리보기만 그려야 한다.');
assert(!previewBody.includes('renderKospiHistoryChart'), '드래그 중 KOSPI 전체 차트를 다시 만들면 안 된다.');
assert(!previewBody.includes('renderKodexHistoryChart'), '드래그 중 KODEX 전체 차트를 다시 만들면 안 된다.');
assert(!previewBody.includes('renderCompositeMomentumChart'), '드래그 중 합성 모멘텀 봉을 다시 만들면 안 된다.');
assert(!previewBody.includes('renderTqqqSynchronized'), '드래그 중 TQQQ 전체 차트를 다시 만들면 안 된다.');
...
assert(dashboard.includes("input.addEventListener('pointerdown', queueTechnicalRangePreview)"));
assert(dashboard.includes("input.addEventListener('pointerup', queueTechnicalRangeRender)"));
assert(dashboard.includes('function renderRangeDependentTechnicalCharts()'));
...
assert(css.includes('.technical-navigator-active .market-dashboard-page {'), '하단 여백은 플로팅 전환과 무관하게 유지돼야 한다.');
assert(css.includes('.technical-range-preview-line {'));
