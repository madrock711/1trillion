...
- 허용 경로는 KOSPI·KOSDAQ, KODEX 레버리지(122630)·삼성전자(005930)·SK하이닉스(000660)의 `basic`·`integration` 응답과 USD/KRW 하나은행 고시환율 응답으로 제한한다. 기술적 분석의 KODEX 차트에는 종목코드와 조회 범위를 고정한 일봉 5개 페이지, TQQQ 비교 차트에는 기간과 간격을 고정한 Yahoo Finance 1년 일봉 응답만 추가로 허용한다. 1년 원자료 중 네비게이터가 선택한 구간을 표시하되 이동평균은 표시 구간을 자르기 전에 계산한다.
...
- 분봉 네비게이터는 선택 거래일을 포함해 최대 네 거래일을 사용하고 기본 손잡이는 최근 두 거래일을 가리킨다. 전체 1분봉을 펼쳐 예상 렌더 봉이 800개를 넘으면 5분 이상으로 자동 집계한다. KOSPI의 외국인 장중 수급은 요청량을 억제하기 위해 최근 두 거래일에만 결합하고 그보다 앞선 구간은 가격·거래량 X-ray만 경량 조회한다.
