...
- 현재 공개 응답에는 체결별 Bid/Ask, 공격 주문 방향, Delta·CVD가 없다. 대신 최근 1분봉의 가격 변화와 거래량을 표준정규 BVC로 배분한 일별 파생값만 `assets/data/kodex-volume-pressure.json`에 누적한다. 첫 분봉, 15:20 이후 종가 단일가와 일봉 대비 미포착 거래량은 50:50으로 중립 처리하고, 추정 매수·매도의 합은 확정 일봉 거래량과 일치시킨다.
- 분봉 포착률이 95% 미만이거나 100.5%를 초과하는 날, 15:30 종가 봉이 없는 날, 분봉 수가 300개 미만인 날은 저장하지 않는다. 기존 날짜는 명시적인 교체 없이 덮어쓰지 않으며, 원본 분봉은 저장소에 보존하지 않는다.
- 공개 차트는 전체 거래량을 빨강으로 그린 뒤 파랑 매도 추정량을 같은 기준선에서 겹쳐, 남은 빨강을 추정 매수 압력으로 보여준다. 이는 실제 체결별 매수·매도량이나 Bid/Ask 체결강도가 아니라 `분봉 기반 추정 매수·매도 압력`으로 표시한다. 추정값이 없는 이전 거래일은 기존 종가 방향별 거래량 막대를 유지한다.
- 매일 평일 16:20 KST의 예약 작업이 최근 6개 완료 세션을 다시 확인해 누락된 날짜만 추가한다. 원천 장애나 휴장으로 새 값이 없으면 기존 누적 파일을 그대로 유지한다.
