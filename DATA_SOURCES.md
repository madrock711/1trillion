...
- 허용 경로는 KOSPI·KOSDAQ, KODEX 레버리지(122630)·삼성전자(005930)·SK하이닉스(000660)의 `basic`·`integration` 응답과 USD/KRW 하나은행 고시환율 응답으로 제한한다. KODEX 전용 차트에는 종목코드와 조회 범위를 고정한 일봉 3개 페이지만 추가로 허용한다. 표시 기간은 3개월이지만 60일 이동평균을 첫 구간부터 계산할 수 있도록 앞선 일봉을 계산용으로 함께 읽는다.
- 응답의 종목 코드, 거래 시각, 거래일, OHLC 범위와 숫자 형식을 검증한 뒤 화면에 반영한다. 하나라도 실패하면 발행 시 검증한 불변 스냅샷을 그대로 표시한다.
- KODEX 레버리지 전용 화면은 같은 `basic`·`integration` 응답에서 현재가·OHLC, 누적 거래량·거래대금, 기간 수익률과 최근 5거래일 투자자별 순매매 수량을 읽는다. 별도 고정 경로의 약 3개월 일봉·거래량은 캔들, 5·20·60일 이동평균과 거래량으로 표시한다. 투자자별 수급표는 가격·거래량 차트와 분리하고 체결 주도 매수·매도 비율로 해석하지 않는다.
