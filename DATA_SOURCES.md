...
- 허용 경로는 KOSPI·KOSDAQ, KODEX 레버리지(122630)·삼성전자(005930)·SK하이닉스(000660)의 `basic`·`integration` 응답과 USD/KRW 하나은행 고시환율 응답으로 제한한다.
- 응답의 종목 코드, 거래 시각, 거래일, OHLC 범위와 숫자 형식을 검증한 뒤 화면에 반영한다. 하나라도 실패하면 발행 시 검증한 불변 스냅샷을 그대로 표시한다.
- KODEX 레버리지 전용 화면은 같은 `basic`·`integration` 응답에서 현재가·OHLC, 누적 거래량·거래대금, 기간 수익률과 최근 5거래일 투자자별 순매매 수량을 읽는다. 투자자별 값은 거래일별 순매매 수량으로 표시하고 체결 주도 매수·매도 비율로 해석하지 않는다.
